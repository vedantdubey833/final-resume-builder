...
const TextField = (props) => {
  ...
  const modifyLabelPosition = (type) => {
    if (val){
      ...
      return setLabelStyles({ bottom: "30px", opacity: 0.9, color: "white" });
    return setLabelStyles({ bottom: "45px", opacity: 0.9, color: "white" })
    }
    if (type === "focusIn"){
      if(window.innerWidth<800)
        return setLabelStyles({ bottom: "30px", opacity: 0.9, color: "white" });
      return setLabelStyles({ bottom: "45px", opacity: 0.9, color: "white" })}
    return setLabelStyles({ bottom: "12px", opacity: 0.8, color: "white" });
  };
